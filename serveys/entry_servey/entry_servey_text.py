...
def format_user_data(user_data: dict) -> str:
    return f'Новая анкета (входная) от пользователя '+str(user_data['user_id'])+' | (Unknown):\n\n'\
            '('+str(user_data['date'])+')\n\n'\
            f'Q (00:00):📍 Вопрос 1: Как часто ты сталкиваешься с проявлением ОКР в среднем?\n'\
            f'A (00:00):'+str(user_data['frequency'])+'\n\n'\
            f'Q (00:00):📍 Вопрос 2: Оцените по 10-балльной шкале, как сильно ОКР влияет на твою повседневную жизнь.\n(1 - не влияет, 10 - оказывается определяющим фактором)\n'\
            f'A (00:00):'+str(user_data['severity'])+'\n\n'
